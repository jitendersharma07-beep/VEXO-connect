...
Used so far: `…800000_vc104_phone_order_centre`, and
`…800001_vc104_slot_count_indexes` (09-25). The second was generated by
`prisma migrate dev --create-only`, which timestamps with the wall clock and so
first landed at `20260925011249` — **outside the range above**. It was renamed
into the range before being committed. Worth noting because the tool will do the
same thing to the next migration on this lane: the range is a convention Prisma
knows nothing about, so it has to be applied by hand each time.
