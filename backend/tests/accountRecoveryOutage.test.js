...
// Dependency-ordered, and deliberately the same list as the sibling suite's.
// An abbreviated version of this passed when the file was run alone — the
// global setup truncates, so there was nothing to collide with — and then
// failed inside the full suite on `Order_openedById_fkey`, because an earlier
// file had left orders pointing at the users this deletes. Every table between
// the challenge rows and PosUser is here for that reason.
const wipe = async () => {
