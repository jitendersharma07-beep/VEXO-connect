...
// point at one database (vcx_kitchen_test). fileParallelism:false makes that
// safe WITHIN a run; it does nothing between runs. Several agents work these
// lanes at once, so two vitest processes can be mid-run against the same
// database — and then each one's wipe() deletes the other's fixtures.
//
// Ported from the x/foundation lane, which caught it in the act on 2026-09-24.
// Both sides logged, and the damage was symmetric:
...
//     be 403": its PosUser rows were gone, so the tokens stopped resolving. The
//     fixture create then hit "Foreign key constraint violated:
//     PosUser_branchId_companyId_fkey" — the branch row had gone too.
//   /tmp/vcx-rl-1-gateway-113244.log — another agent, "Start at 11:32:44",
//     inside that window. Its OWN wipe() at tests/gateway.test.js:67 failed with
//     "Region_companyId_fkey": it was deleting companies while the first run
//     still had regions pointing at them.
// Neither report had anything to do with the code under test. This lane has the
// identical shape — 11 of 14 suites wipe in beforeAll against one database, and
// tests/gateway.test.js is the same file — so it has the identical exposure.
//
...
// Contention here is low (runs are ~1 min), so waiting is cheap and the
...
// 0x564358, "VCX". Deliberately the same key the foundation lane uses: advisory
// locks are scoped to a database, so lanes with their own *_test database never
// contend and the key needs no per-lane derivation. Verified here rather than
// assumed, because both lanes live on ONE server (127.0.0.1:5440) and only the
// database differs — holding this key's stand-in on vcx_kitchen_test, a second
// session on the same database got `f` and a session on vcx_foundation_test got
// `t`, and the pg_locks row carried the database oid (28936 = vcx_kitchen_test).
