...
describe('request-log credential redaction', () => {
  ...
  describe('redact paths', () => {
    ...
    describe('an approval password, however deep it is nested', () => {
      ...
      // Every case above nests the password under a key called `approval`, so
      // they would all still pass if the ladder were replaced by the narrower
      // '*.approval.password'. Two independent fixes for this defect were
      // written — one keyed on depth, one on that name — and the depth one was
      // kept precisely because the bug is "a password below level one", not
      // "a password under `approval`". This is the case that tells the two
      // apart, and it is the one that fails if somebody tidies the ladder
      // back down to the name.
      it('is censored under a key that is not named approval', () => {
        const line = emit({ req: { body: { manager: { password: CANARY } } } });
        expect(line.req.body.manager.password).toBe('[REDACTED]');
        expect(JSON.stringify(line)).not.toContain(CANARY);
      });
    });
  });
